...
function xScale(stateHealthData, chosenXAxis) {
  ...
  var xLinearScale = d3.scaleLinear()
    .domain([d3.min(stateHealthData, d => d[chosenXAxis]) * 0.8,
      d3.max(stateHealthData, d => d[chosenXAxis]) * 1.2
    ])
    .range([0, width]);
...
function yScale(stateHealthData, chosenYAxis) {
  ...
  var yLinearScale = d3.scaleLinear()
     .domain([d3.min(stateHealthData, d => d[chosenYAxis]) * 0.8,
     d3.max(stateHealthData, d => d[chosenYAxis]) * 1.2])
     .range([height, 0]);
...
function updateToolTip(chosenXAxis, circlesGroup) {
  ...
  var toolTip = d3.tip()
    .attr("class", "tooltip")
    .offset([-8, 0])
    .html(d => `<h6>${d.state}</h6><hr><h6> <br>${xLabel}: ${d[chosenXAxis]}</h6> <br>${yLabel}: ${d[chosenYAxis]}`);
  ...
  labelYGroup.selectAll("text")
    .on("click", function() {
      ...
      if (value !== chosenYAxis) {
        ...
        textsGroup = renderTexts(textsGroup, yLinearScale, chosenYAxis);
